	...
	},
	{
		...
		stmts: []string{
			`CREATE TABLE IF NOT EXISTS sites (
				...
				reading_url         TEXT NOT NULL DEFAULT '',
				checkin_button_rule TEXT NOT NULL DEFAULT '',
			...
			`CREATE TABLE IF NOT EXISTS site_accounts (
				...
				checkin_url            TEXT NOT NULL DEFAULT '',
				checkin_button_rule    TEXT NOT NULL DEFAULT '',
				auto_read_enabled      INTEGER NOT NULL DEFAULT 0,
				reading_url            TEXT NOT NULL DEFAULT '',
			...
			`CREATE INDEX IF NOT EXISTS idx_site_accounts_profile_id ON site_accounts(profile_id)`,
			`CREATE INDEX IF NOT EXISTS idx_site_accounts_status ON site_accounts(status)`,
			`CREATE TABLE IF NOT EXISTS site_account_task_runs (
			...
			`CREATE INDEX IF NOT EXISTS idx_site_account_task_runs_account_id ON site_account_task_runs(account_id)`,
			`CREATE INDEX IF NOT EXISTS idx_site_account_task_runs_started_at ON site_account_task_runs(started_at)`,
		},
	},
	// ── 新版本在此追加，格式：
	// {
